...
from torchmetrics.functional import dice
import os
#If True, will display a visual example of the segmentation instead of testing all the test set to compute metric
VISUAL = False
metric = load_metric("mean_iou")

net = greedyvig_b_feat()
...
if VISUAL :
    ...
    with torch.no_grad():
        ...
        for j, data in enumerate(tqdm(test_dataloader, 0)):
            ...
            dice_metric = dice(outputs.detach().cpu(),labels.detach().cpu(),average = None,num_classes=4)
