...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(NB_EPOCHS)):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(train_dataloader, 0):
            ...
            #TODO Try focal loss
            # loss += .75 * criterion(out1, labels.type(torch.LongTensor).to(device))
            # loss += .375 * criterion(out2, labels.type(torch.LongTensor).to(device))
            # loss += .1875 * criterion(out3, labels.type(torch.LongTensor).to(device))
